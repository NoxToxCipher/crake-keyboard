...
impl RadixTrie {
    ...
    /// Top-`limit` words under `prefix` (frequency desc, then lex asc)
    /// that satisfy `keep`, filtered DURING traversal — the glide pool
    /// used to pull 1500 sorted clones per start letter just to discard
    /// most of them by end-letter (1.27ms/call for "s", measured
    /// 2026-08-27, on the mid-gesture preview path).
    pub fn prefix_search_filtered(
        &self,
        prefix: &str,
        limit: usize,
        ...
        let mut top: Vec<(&String, u32)> = Vec::with_capacity(limit + 1);
        if let Some(node) = self.get_terminal_node(prefix) {
            self.collect_top_filtered(node, limit, &keep, &mut top);
        }
        top.into_iter().map(|(w, f)| (w.clone(), f)).collect()
    }

    fn collect_top_filtered<'a>(
        &'a self,
        node: &'a TrieNode,
        limit: usize,
        keep: &impl Fn(&str) -> bool,
        top: &mut Vec<(&'a String, u32)>,
    ) {
        if node.is_terminal {
