...
impl NlpEngine {
    ...
    pub fn suggest_with_context_opts(
            ...
            .find(|&&(k, _)| k == trimmed_lower)
        {
            ...
            // A name written with its fada is the name ("Roisin" is
            // "Roisin" typed on a phone, "Róisín" is how it is spelt), so
            // offer the plain capital beside the accented one rather than
            // choosing for the person (field report 2026-09-19).
            let plain_capital = Self::apply_casing("Xx", &trimmed_lower);
            if plain_capital != brand_casing
                && !candidates.iter().any(|c| c.word.eq_ignore_ascii_case(&plain_capital))
            {
                candidates.push(RankedCandidate {
                    word: plain_capital,
